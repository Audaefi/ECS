...
class LazadaPhSpider(scrapy.Spider):
    name = 'lazada_ph'
    ...
    def start_requests(self):
        ...
        for keyword in keyword_list:
            for page in range(1, pages + 1):
                search_url = f"https://www.lazada.com.ph/catalog/?_keyori=ss&from=input&page={page}&q={keyword}"
    ...
    def parse_product(self, response):
        ...
        product_price = response.css(
            '.pdp-price.pdp-price_type_normal.pdp-price_color_orange.pdp-price_size_xl ::Text').get()
        product_seller = response.css(
            '.pdp-link.pdp-link_size_l.pdp-link_theme_black.seller-name__detail-name ::Text').get()
